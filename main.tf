...
# --- Provider Kubernetes ---
# Configur├⌐ dynamiquement ├á partir des credentials AKS
provider "kubernetes" {
...
# --- Provider Helm ---
# Utilis├⌐ pour installer ArgoCD via Helm chart
provider "helm" {
  kubernetes = {
    host                   = module.aks.kube_config_host
    client_certificate     = base64decode(module.aks.kube_config_client_certificate)
...
# --- Provider kubectl ---
# Utilis├⌐ pour cr├⌐er les CRDs ArgoCD Application
provider "kubectl" {
  host                   = module.aks.kube_config_host
  client_certificate     = base64decode(module.aks.kube_config_client_certificate)
  client_key             = base64decode(module.aks.kube_config_client_key)
...
# D├⌐pend du module network pour le subnet
# =============================================================================
module "aks" {
...
# Install├⌐ via Helm sur le cluster AKS
# =============================================================================
module "argocd" {
...
# =============================================================================
# Module : Keycloak (IAM/SSO - d├⌐ploy├⌐ via Kubernetes natif)
...
# =============================================================================
# Module : Odoo (ERP - d├⌐ploy├⌐ via Kubernetes natif)
